...
# v22: crystal-rotation (rotate_crystal_mode=True) data — grazing-free, crystal-frame,
# corrected Pool B (standard axes). Merged train = Pool A (ion<300k) + Pool B (ion>=300k).
ROT_DIR       = BASE_DIR / "data/siimpl_rot"
TRAIN_CSV     = ROT_DIR / "siimpl_train.csv"
EVAL_CSV      = ROT_DIR / "siimpl_eval_merged.csv"
...
# Tier-0 (v22): sign-aware auxiliary annealing. The axis-aware aux loss min(NLL(+d),
# NLL(-d)) rewards ORIENTATION-INVARIANT features, which shapes the encoder against
# head-tail exactly when it dominates (alpha=0.5) early. Use it for stability during
# the first AUX_SIGN_WARMUP epochs, then switch to the SIGNED vMF NLL so head-tail
# information reaches the backbone. (Not the failed parity head: same aux objective,
# just no longer sign-symmetrized.)
AUX_SIGN_WARMUP = 30

# Stage 2: flow-only fine-tuning
STAGE2_EPOCHS    = 80     # backstop; STAGE2_PATIENCE decides (same principle as Stage 1)
STAGE2_LR_MAX    = 3e-4   # Tier-0 (v22): was 5e-5 — a fine-tune LR on a FRESH head is
                          # undertraining; the deployed posterior comes from Stage 2.
                          # Train the fresh head at from-scratch LR (cosine+warmup).
STAGE2_LR_MIN    = 1e-6
STAGE2_WARMUP    = 3
STAGE2_PATIENCE  = 20
...
def train_one_epoch(flow, cached_embedding, dir_head, energy_head,
    ...
    for batch_idx, (theta_batch, x_batch) in enumerate(train_loader):
        ...
        # 3. Auxiliary direction loss — Tier-0 (v22): sign-aware annealing.
        target_dir = theta_batch[:, 1:4]  # (batch, 3) — Vx, Vy, Vz
        mu_hat, kappa = dir_head(z)
        if epoch < AUX_SIGN_WARMUP:
            dir_nll = axis_aware_vmf_nll(mu_hat, kappa, target_dir)   # stabilize early
        else:
            dir_nll = vmf_nll(mu_hat, kappa, target_dir)             # signed: learn head-tail
...
def main():
    ...
    # Tier-0 (v22): seed all RNGs so the config split is deterministic across
    # invocations (prevents --resume from moving previously-trained configs into
    # the val set and corrupting early stopping) and ablations are attributable.
    import random as _random
    SEED = 42
    torch.manual_seed(SEED); np.random.seed(SEED); _random.seed(SEED)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(SEED)
    print(f"[SEED] all RNGs seeded to {SEED}")
