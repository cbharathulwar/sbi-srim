...
def evaluate_one_random_mnpe(
    ...
    *,
    ...
    print(f"[INFO][MNPE] True random energy = {E_true:.3f} keV")
    ...
    # -----------------------------
    # FIX: Assign parity PER TRACK
    # -----------------------------
    ion_ids = sorted(df["ion_number"].unique().tolist())
    df["parity"] = df["ion_number"].apply(lambda t: random.choice([0,1]))

    # Flip per ion
    df.loc[df["parity"] == 0, "x"] *= -1

    # -----------------------------
    # FIX: Center each ion separately
    # -----------------------------
    centered_parts = []
    for ion in ion_ids:
    ...
    df = pd.concat(centered_parts, ignore_index=True)

    # Save combined CSV
    csv_path = Path(run_dir) / f"{E_true:.3f}keV_randomall.csv"
    df.to_csv(csv_path, index=False)

    # Extract features for all tracks
    x_obs_all, _, track_ids, _, _ = preprocess_mnpe(csv_path, n_bins=n_bins)

    # Predict for each ion
    results = []
    for i in range(len(x_obs_all)):
        x = x_obs_all[i].unsqueeze(0)
        ion = df[df["ion_number"] == i]

        true_parity = int(ion["parity"].iloc[0])
